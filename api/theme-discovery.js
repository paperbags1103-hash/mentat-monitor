...
const THEME_SEEDS = [
  {
    ...
    koreanStocks: ['한화에어로스페이스', 'LIG넥스원', '현대로템'],
    usStocks: ['LMT', 'RTX', 'NOC', 'GD'],
  },
  {
    id: 'ai_agent_infra',
    nameKo: 'AI 에이전트 인프라',
    keywords: ['AI 에이전트', 'MCP', 'LLM', 'HBM', '데이터센터', '엔비디아', 'SK하이닉스'],
    koreanStocks: ['SK하이닉스', '삼성전자', 'TIGER AI반도체'],
    usStocks: ['NVDA', 'MSFT', 'PLTR', 'ORCL'],
  },
  {
    id: 'korea_valup_foreigners',
    nameKo: '코스피 밸류업',
    keywords: ['밸류업', '외국인 순매수', '코스피', '탄핵', '정치리스크 해소', '저PBR'],
    koreanStocks: ['KODEX 200', '삼성전자', '현대차', 'KB금융'],
    usStocks: ['EWY', 'FLKR'],
  },
  {
    ...
    koreanStocks: ['KODEX 미국달러선물', '내수주'],
    usStocks: ['UUP', 'DXY', 'VIXY'],
  },
  {
    id: 'nuclear_ai_power',
    nameKo: '원전·AI 전력 인프라',
    keywords: ['원전', '데이터센터 전력', '전력망', 'SMR', '두산에너빌리티', 'LS ELECTRIC'],
    koreanStocks: ['두산에너빌리티', 'LS ELECTRIC', '한전기술'],
    usStocks: ['CEG', 'VST', 'NRG', 'OKLO'],
  },
  {
    id: 'hbm4_memory_race',
    nameKo: 'HBM4 메모리 경쟁',
    keywords: ['HBM4', 'HBM3e', '고대역폭메모리', 'SK하이닉스', '삼성전자', '마이크론', 'AI반도체'],
    koreanStocks: ['SK하이닉스', '삼성전자', 'TIGER 반도체'],
    usStocks: ['MU', 'AMAT', 'LRCX', 'KLAC'],
  },
  {
    id: 'china_ai_rise',
    nameKo: '중국 AI 굴기',
    keywords: ['딥시크', 'DeepSeek', '중국AI', '알리바바AI', 'Tencent', '바이두', 'Kimi'],
    koreanStocks: ['TIGER 차이나과창판TECH', 'KODEX 차이나H'],
    usStocks: ['BIDU', 'KWEB', 'BABA', 'PDD'],
  },
  {
    id: 'bitcoin_institutionalization',
    nameKo: '비트코인 기관화',
    keywords: ['비트코인', '현물ETF', '기관투자', '코인베이스', '마이크로스트래티지', '암호화폐'],
    koreanStocks: ['TIGER 미국비트코인ETF', 'KODEX 비트코인선물'],
    usStocks: ['IBIT', 'COIN', 'MSTR', 'FBTC'],
  },
];
...
[
  {
    ...
    "riskKo": ["리스크1", "리스크2"],
    "koreanStocks": ["한국 상장 종목 또는 ETF (KODEX·TIGER 포함) 2-4개"],
    "usStocks": ["NYSE·NASDAQ 티커 2-4개 (예: NVDA, LMT, XOM)"],
    "updatedAt": ${Date.now()}
  }
]
...
function generateTemplateThemes(signals, inferences) {
  ...
  const scored = THEME_SEEDS.map(seed => {
    ...
    return {
      ...
      beneficiaryKo: [...(seed.koreanStocks ?? []).slice(0, 2), ...(seed.usStocks ?? []).slice(0, 2)],
      riskKo:       ['글로벌 리스크온 전환', '달러강세 전환'],
      koreanStocks: (seed.koreanStocks ?? []).slice(0, 4),
      usStocks:     (seed.usStocks ?? []).slice(0, 4),
      updatedAt:   Date.now(),
    };
  });
